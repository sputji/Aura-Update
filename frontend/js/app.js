...
async function runCoolBoost() {
    ...
    try {
        ...
        if (result.log && result.log.length > 0) {
            ...
            const formattedLog = formatCoolBoostLog(result.log.join('\n'));
            logEl.textContent = formattedLog;
        }
...
function collectTurboMatrixSelection() {
    const rows = Array.from(document.querySelectorAll('.turbo-proc-row'));
    const whitelist = [];
    const blacklist = [];

    for (const row of rows) {
        const name = (row.dataset.procName || '').toLowerCase();
        const keep = row.querySelector('input[data-role="keep"]');
        const close = row.querySelector('input[data-role="close"]');
        if (!name || !keep || !close) continue;
        if (keep.checked) whitelist.push(name);
        if (close.checked) blacklist.push(name);
    }

    return { whitelist, blacklist };
}

function applyTurboProfileToMatrix(profile) {
    if (!profile) return;
    const whitelist = (profile.whitelist || []).map(v => String(v).toLowerCase());
    const blacklist = (profile.blacklist || []).map(v => String(v).toLowerCase());

    document.querySelectorAll('.turbo-proc-row').forEach((row) => {
        const proc = (row.dataset.procName || '').toLowerCase();
        const keep = row.querySelector('input[data-role="keep"]');
        const close = row.querySelector('input[data-role="close"]');
        if (!keep || !close) return;

        keep.checked = whitelist.some(v => proc.includes(v));
        close.checked = !keep.checked && blacklist.some(v => proc.includes(v));
    });
}

function renderTurboProcessMatrix(processes) {
    const container = $('#turboProcessMatrix');
    if (!container) return;

    if (!processes || processes.length === 0) {
        container.innerHTML = `<p class="status-desc">${t('procs_welcome_desc')}</p>`;
        return;
    }

    container.innerHTML = processes.map((p) => `
        <div class="turbo-proc-row" data-proc-name="${escapeHtml(p.name)}">
            <span class="turbo-proc-name">${escapeHtml(p.name)}</span>
            <span class="turbo-proc-meta">CPU ${Number(p.cpu_percent || 0).toFixed(1)}% · RAM ${Number(p.memory_mb || 0)} MB</span>
            <label><input type="checkbox" data-role="keep"> ${t('turbo_keep') || 'Garder'}</label>
            <label><input type="checkbox" data-role="close"> ${t('turbo_close') || 'Fermer'}</label>
        </div>
    `).join('');

    container.querySelectorAll('input[data-role="keep"]').forEach((cb) => {
        cb.addEventListener('change', (e) => {
            if (!e.target.checked) return;
            const row = e.target.closest('.turbo-proc-row');
            const close = row?.querySelector('input[data-role="close"]');
            if (close) close.checked = false;
            updateTurboProfileSummary();
        });
    });
    container.querySelectorAll('input[data-role="close"]').forEach((cb) => {
        cb.addEventListener('change', (e) => {
            if (!e.target.checked) return;
            const row = e.target.closest('.turbo-proc-row');
            const keep = row?.querySelector('input[data-role="keep"]');
    ...
    const selected = $('#turboProfileSelect')?.value;
    if (selected) {
        const profile = (state.turboProfiles || []).find(p => p.name === selected);
        if (profile) applyTurboProfileToMatrix(profile);
    }
}

async function loadTurboProcessesForMatrix() {
    const container = $('#turboProcessMatrix');
    if (!container) return;
    container.innerHTML = `<p class="status-desc">${t('scanning')}</p>`;
    try {
        const processes = await invoke('get_heavy_processes');
        renderTurboProcessMatrix(processes || []);
    } catch (e) {
        container.innerHTML = `<p class="status-desc">${t('error_load')}: ${escapeHtml(String(e))}</p>`;
    }
}

function applyTurboPreset(presetKey) {
    const presets = {
        game: {
            whitelist: ['discord', 'steam', 'battle.net'],
            blacklist: ['chrome', 'teams', 'spotify', 'onedrive'],
        },
        stream: {
            whitelist: ['obs64', 'discord', 'streamlabs'],
            blacklist: ['chrome', 'teams', 'adobe', 'onedrive'],
        },
        work: {
            whitelist: ['teams', 'outlook', 'slack'],
            blacklist: ['spotify', 'xbox', 'gamebar', 'steam'],
        },
    };
    const preset = presets[presetKey];
    if (!preset) return;

    $('#turboWhitelist').value = preset.whitelist.join(',');
    $('#turboBlacklist').value = preset.blacklist.join(',');
    applyTurboProfileToMatrix(preset);
}

function reorganizeTurboCards() {
    const cleanupTab = $('#tab-cleanup');
    const maintenanceGrid = $('#tab-maintenance .maintenance-grid');
    if (!cleanupTab || !maintenanceGrid) return;

    const browserGranular = $('#browserGranularSection');
    const predictiveCard = $('#predictiveValue')?.closest('.turbo-card');
    const bloatCard = $('#btnPurgeBloat')?.closest('.turbo-card');
    const backupsCard = $('#backupsList')?.closest('.turbo-card');

    let cleanupHost = $('#cleanupEnhancements');
    if (!cleanupHost) {
        cleanupHost = document.createElement('div');
        cleanupHost.id = 'cleanupEnhancements';
        cleanupHost.className = 'turbo-grid';
        cleanupHost.style.marginTop = '14px';
        cleanupTab.appendChild(cleanupHost);
    }

    if (browserGranular) cleanupHost.appendChild(browserGranular);
    if (predictiveCard) cleanupHost.appendChild(predictiveCard);

    if (bloatCard && !bloatCard.closest('.maintenance-card')) {
        const wrap = document.createElement('div');
        wrap.className = 'maintenance-card';
        wrap.appendChild(bloatCard);
        maintenanceGrid.appendChild(wrap);
    }
    if (backupsCard && !backupsCard.closest('.maintenance-card')) {
        const wrap = document.createElement('div');
        wrap.className = 'maintenance-card';
...
function fillTurboProfileEditor(name) {
    ...
    applyTurboProfileToMatrix(profile);
}
...
async function saveTurboProfileFromUI() {
    ...
    const matrixSelection = collectTurboMatrixSelection();
    const whitelist = matrixSelection.whitelist.length
        ? matrixSelection.whitelist
        : parseCsvList($('#turboWhitelist')?.value || '');
    const blacklist = matrixSelection.blacklist.length
        ? matrixSelection.blacklist
        : parseCsvList($('#turboBlacklist')?.value || '');
...
/* ─── Cool Boost Log Formatting ────────────────────────────── */
function formatCoolBoostLog(output) {
    if (!output) return '';
    const lines = output.split('\n');
    // Filter out "SKIP: Not available" lines to reduce noise
    const filtered = lines.filter(line => !line.includes('SKIP: Not available'));
    return filtered.join('\n').trim();
}

/* ─── Turbo Profile Clarity ────────────────────────────────── */
function updateTurboProfileSummary() {
    const summaryEl = $('#turboProfileSummary');
    if (!summaryEl) return;
    
    const keepCheckboxes = document.querySelectorAll('.turbo-proc-row input[data-role="keep"]:checked');
    const closeCheckboxes = document.querySelectorAll('.turbo-proc-row input[data-role="close"]:checked');
    const keepCount = keepCheckboxes.length;
    const closeCount = closeCheckboxes.length;
    
    if (keepCount === 0 && closeCount === 0) {
        summaryEl.textContent = t('turbo_profile_summary_empty') || 'Analysez les processus pour afficher un résumé clair du profil.';
    } else {
        summaryEl.textContent = `Keep: ${keepCount}, Close: ${closeCount}`;
    }
}

/* ─── Update Install Directory ─────────────────────────────── */
async function chooseUpdateInstallDir() {
    try {
        const { open } = window.__TAURI__.dialog || window.__TAURI_PLUGIN_DIALOG__;
        const selected = await open({ directory: true, multiple: false, title: 'Select Update Installation Directory' });
        if (selected) {
            await invoke('set_config_value', { key: 'update_install_dir', value: selected });
            state.config.update_install_dir = selected;
            await syncUpdateInstallDirUI();
            showToast(t('update_install_dir_set') || 'Update directory configured', 'success');
        }
    } catch (e) {
        showToast(t('error') + ': ' + e, 'error');
    }
...
async function syncUpdateInstallDirUI() {
    const pathEl = $('#updateInstallDirPath');
    const resetBtn = $('#btnResetUpdateInstallDir');
    if (pathEl) {
        const dir = (state.config && state.config.update_install_dir) || '';
        if (dir) {
            pathEl.textContent = dir;
            if (resetBtn) resetBtn.classList.remove('hidden');
        } else {
            pathEl.textContent = t('settings_update_install_dir_auto') || 'Automatic (current app directory)';
            if (resetBtn) resetBtn.classList.add('hidden');
        }
    }
}

async function initUpdateInstallDir() {
    await syncUpdateInstallDirUI();
}
...
function bindEvents() {
    ...
    if ($('#btnTurboLoadProcesses')) {
        $('#btnTurboLoadProcesses').addEventListener('click', loadTurboProcessesForMatrix);
    }
    if ($('#btnPresetGame')) {
        $('#btnPresetGame').addEventListener('click', () => applyTurboPreset('game'));
    }
    if ($('#btnPresetStream')) {
        $('#btnPresetStream').addEventListener('click', () => applyTurboPreset('stream'));
    }
    if ($('#btnPresetWork')) {
        $('#btnPresetWork').addEventListener('click', () => applyTurboPreset('work'));
    }
    ...
    // Update install directory
    const btnChooseUpdateInstallDir = $('#btnChooseUpdateInstallDir');
    if (btnChooseUpdateInstallDir) btnChooseUpdateInstallDir.addEventListener('click', chooseUpdateInstallDir);
    const btnResetUpdateInstallDir = $('#btnResetUpdateInstallDir');
    if (btnResetUpdateInstallDir) {
        btnResetUpdateInstallDir.addEventListener('click', async () => {
            await invoke('set_config_value', { key: 'update_install_dir', value: '' });
            state.config.update_install_dir = '';
            await syncUpdateInstallDirUI();
            showToast(t('update_install_dir_reset') || 'Update directory reset to automatic', 'success');
        });
    }
...
async function init() {
    ...
    try {
        ...
        loadSystemSpecs();
        initBackupDir();
        initUpdateInstallDir();
        loadTurboProfiles();
        reorganizeTurboCards();
        refreshBatteryMode();
